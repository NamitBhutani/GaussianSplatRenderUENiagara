// PLYParser.cpp
// Implementation of PLY file parser for Gaussian Splat data
...
FPLYParser::FPLYParser()
	...
	, VertexCount(0)
	, PropIdx_X(-1), PropIdx_Y(-1), PropIdx_Z(-1)
	, PropIdx_NX(-1), PropIdx_NY(-1), PropIdx_NZ(-1)
	, PropIdx_FDC0(-1), PropIdx_FDC1(-1), PropIdx_FDC2(-1)
	, PropIdx_Opacity(-1)
	, PropIdx_Scale0(-1), PropIdx_Scale1(-1), PropIdx_Scale2(-1)
	, PropIdx_Rot0(-1), PropIdx_Rot1(-1), PropIdx_Rot2(-1), PropIdx_Rot3(-1)
{
}
...
bool FPLYParser::ParseFile(const FString& FilePath, TArray<FGaussianSplatData>& OutSplats)
{
	...
	else if (Format == EPLYFormat::BinaryLittleEndian || Format == EPLYFormat::BinaryBigEndian)
	{
		...
		{
			HeaderByteOffset += Lines[i].Len() + 1; // +1 for newline (assumes LF, may need adjustment for CRLF)
		}

		// Adjust for CRLF if needed
		FString TestString;
...
bool FPLYParser::ParseASCIIData(const TArray<FString>& Lines, int32 StartLine, TArray<FGaussianSplatData>& OutSplats)
{
	...
	for (int32 i = 0; i < VertexCount; ++i)
	{
		...
		{
			ErrorMessage = FString::Printf(TEXT("Not enough values at vertex %d (expected %d, got %d)"), i, NumProperties, Tokens.Num());
			return false;
		}
...
bool FPLYParser::ParseBinaryData(const TArray<uint8>& FileData, int32 HeaderByteOffset, TArray<FGaussianSplatData>& OutSplats)
{
	OutSplats.Reserve(VertexCount);
	...
	for (int32 i = 0; i < VertexCount; ++i)
	{
		...
		for (int32 j = 0; j < NumProperties; ++j)
		{
			...
			{
				// Handle other types as needed, skip for now
...
FGaussianSplatData FPLYParser::ExtractSplatData(const TArray<float>& PropertyValues)
{
	...
	{
		Splat.Normal = FVector(PropertyValues[PropIdx_NX], PropertyValues[PropIdx_NY], PropertyValues[PropIdx_NZ]);
	}
	...
	if (PropIdx_Scale0 != INDEX_NONE && PropIdx_Scale1 != INDEX_NONE && PropIdx_Scale2 != INDEX_NONE)
	{
		Splat.Scale = FGaussianSplatData::ConvertScaleToUnreal(
			PropertyValues[PropIdx_Scale0],
			PropertyValues[PropIdx_Scale1],
			PropertyValues[PropIdx_Scale2]
		);
	}

	// Orientation (PLY stores as w, x, y, z in rot_0, rot_1, rot_2, rot_3)
	if (PropIdx_Rot0 != INDEX_NONE && PropIdx_Rot1 != INDEX_NONE && PropIdx_Rot2 != INDEX_NONE && PropIdx_Rot3 != INDEX_NONE)
	{
		Splat.Orientation = FGaussianSplatData::ConvertOrientationToUnreal(
			PropertyValues[PropIdx_Rot0], // W
	...
	// Zero order spherical harmonics (base color)
	if (PropIdx_FDC0 != INDEX_NONE && PropIdx_FDC1 != INDEX_NONE && PropIdx_FDC2 != INDEX_NONE)
	{
		Splat.ZeroOrderHarmonicsCoefficients = FVector(
			PropertyValues[PropIdx_FDC0],
			PropertyValues[PropIdx_FDC1],
			PropertyValues[PropIdx_FDC2]
		);
	}
	...
	if (PropIdx_FRest.Num() > 0)
	{
		...
		{
			Splat.HighOrderHarmonicsCoefficients.Add(FVector(
				PropertyValues[PropIdx_FRest[i]],
